    ...
    public class KillStreamScheduledTask : IScheduledTask, IConfigurableScheduledTask
    {
        ...
        public async Task Execute(CancellationToken cancellationToken, IProgress<double> progress)
        {
            ...
            foreach (var sessionManagerSession in SessionManager.Sessions)
            {
                if (sessionManagerSession.PlayState.PlayMethod == PlayMethod.Transcode && sessionManagerSession.NowPlayingItem != null)
                {
                        ...
                        !Plugin.Instance.PluginConfiguration.Allow4KVideoTranscode))
                    {
                        ...
                        await SessionManager.SendMessageCommand(null, sessionManagerSession.Id,
                            new MessageCommand
                            {
                                Header = "4K Stream Transcoding Disabled",
                ...
                if (sessionManagerSession.PlayState.PlayMethod == PlayMethod.Transcode && !sessionManagerSession.TranscodingInfo.IsVideoDirect &&
            ...
            foreach (var pausedStream in killPausedStreams)
            {
                ...
                var text = "Stream killed due to being paused for more than " + Plugin.Instance.PluginConfiguration.PausedDurationMin +"min.";
        ...
        public string prettyText(string input)
        {
            ...
            {
                input = input.Replace("ContainerBitrateExceedsLimit", "The file is in a higher quality than your selected play back quality.  ");
            }
            ...
            if (input.Contains("DirectPlayError"))
            {
                input = input.Replace("DirectPlayError", "Direct play error.  ");
            }
